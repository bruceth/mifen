...
export class VHistoryExplorer extends View<CHistoryExplorer> {
  ...
  protected rowContent = observer((row: any): JSX.Element => {
    ...
    let { id, name, code, e, capital, bonus, price, zf1, zf2, zf3, order, b, r2, l, lr2, lr4, b10, r210, l10, lr210, predictpe } = row as NStockInfo;
    let pe = price / e;
    let roe = e / capital;
    ...
    if (fInBlack >= 0)
      ...
      return <><LMR className="px-1 py-1" left={left} right = {right} >
        <div className="d-flex flex-wrap" onClick={()=>this.onClickName(row)} >
          ...
          <div className="px-3 c6">{GFunc.caption('价/预期')}<br />{GFunc.percentToFixString(predictpe)}</div>
